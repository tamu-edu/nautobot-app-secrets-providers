...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SECRETS_PROVIDERS_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_secrets_providers.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_secrets_providers.ephemeral_ports
        and context.nautobot_secrets_providers.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_secrets_providers.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_secrets_providers.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_secrets_providers.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_secrets_providers.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_secrets_providers.compose_dir, "creds.env"),
        )
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_secrets_providers.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_secrets_providers"
...
@task(
    help={
        "version": "Version of Secrets Providers to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_secrets_providers'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_secrets_providers")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_secrets_providers.migrations"
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_secrets_providers/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Secrets Providers."""
    command = "nautobot-server generate_nautobot_secrets_providers_test_data"
    if database:
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_secrets_providers",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def tests(context, failfast=False, keepdb=False, no_input=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_secrets_providers.local):
